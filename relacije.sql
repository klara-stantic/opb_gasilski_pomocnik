...
create table opravljeni_tecaji (
    ...
    emso_udelezenca integer not null REFERENCES clani(emso)
);
...
create table prisotnost_na_intervencijah (
    ...
    emso_prisotnega integer not null REFERENCES clani(emso)
);
...
create table lastnistva_opreme (
    id serial PRIMARY KEY,
    id_lastnika integer REFERENCES clani(emso) DEFAULT 0,
